...
- Tela **Mapa dos votos** (município): em cada bairro, um mini gráfico de barras com os 3 mais votados (foto no topo da barra, quantidade de votos e barra na cor do partido, altura relativa ao 1º do bairro); com candidato selecionado, uma barra só com os votos dele. Clicar no bairro destaca o quadro e o rodapé do mapa passa a mostrar os totais daquele bairro (aptos, comparecimento, válidos, brancos, nulos, seções) com os links "Ver seções" e "Limpar seleção"; sem seleção, o rodapé traz os totais do município. Não há mais painel lateral com pizza e lista.
