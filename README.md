...
- **Onde ficam os dados**: num banco na nuvem ([Supabase](https://supabase.com), projeto `eleicoes-ceara`), configurado em `data/config.json` (URL do projeto e chave pública). Qualquer visitante lê; para editar é preciso **entrar** (botão no topo das telas de 2026) com e-mail e senha ou com um link de acesso enviado por e-mail. Só e-mails convidados pelo administrador no painel do Supabase (Authentication › Users › Add user › Send invitation) conseguem entrar; o cadastro aberto está desligado. Depois de entrar pelo link de convite, "Definir senha" permite acessar com senha. As tabelas e as regras de acesso estão em `scripts/supabase.sql` (leitura pública, gravação só para usuários autenticados). Cada alteração é gravada na hora, linha a linha; as lideranças de um município são carregadas quando ele é aberto, e os candidatos a vereador e prefeito de 2024 são importados na primeira abertura por um usuário conectado.
